...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function applyTransform() {
    lightboxImg.style.transform = `translate(calc(-50% + ${posX}px), calc(-50% + ${posY}px)) scale(${scale})`;
  ...
  lightboxImg.addEventListener("touchmove", e => {
    ...
    if (e.touches.length === 2) {
      ...
      const newScale = Math.min(Math.max(1, initialScale * (dist / initialDistance)), 4);

      const mid = getMidpoint(e.touches[0], e.touches[1]);
      const centerX = window.innerWidth / 2;
      const centerY = window.innerHeight / 2;
      const midOffsetX = mid.x - centerX;
      const midOffsetY = mid.y - centerY;

      const scaleRatio = newScale / scale;
      posX = posX + midOffsetX * (1 - scaleRatio);
      posY = posY + midOffsetY * (1 - scaleRatio);
  ...
  lightboxImg.addEventListener("touchend", e => {
    ...
    if (scale === 1 && e.changedTouches && e.changedTouches[0]) {
